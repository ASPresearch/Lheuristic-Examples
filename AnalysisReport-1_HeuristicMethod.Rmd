...
author: "Berta Miró, Francesc Carmona,  Diego Arango and Alex Sánchez Pla"
...
This document contains examples / applications on how to select L-shaped genes from two data matrices, an expression and a methylation matrix, matched by rows (genes) and columns(samples), using the algorithms implemented by the authors in the `Lheuristic` package and described in section 3, *Scoring Scatterplots*.
...
```{r loadpackages}
if (!(require(Lheuristic))) devtools::install_github("aspresearch/Lheuristic")
...
This can be checked using a simple function such as `checkPairing` available in the package.
...
# Expression-Methylation scatterplots
...
# Separating TRUE and FALSE L-shaped genes
...
## Simulating L-shaped scatterplots

An algorithm to simulate L-shaped genes has been developed in order to test for the capability of the scoring system to discriminate between TRUE or FALSE L-Shape
...
# Examples (1): Scoring the TRUE/FALSE DA dataset
...
(reqPercentages <- matrix (c(10, 20, 0, 5, 0, 30, 10, 5, 20), nrow=3, byrow=TRUE))
sum(reqPercentages)
...
# Examples (1):  Putting all together to select L-shaped genes
...
reqPercentages4TCGA <- matrix (c(4, 20, 5, 2, 40, 20, 1, 3, 5), nrow=3, byrow=TRUE)
sum(reqPercentages4TCGA)
...
theGenes <- 1:numGenes
reqPercentages <- matrix (c(3, 20, 5, 2, 40, 20, 1, 3, 6), nrow=3, byrow=TRUE)
sum(reqPercentages)
